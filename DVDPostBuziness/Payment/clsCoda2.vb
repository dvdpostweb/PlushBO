...
Public Class clsCoda2
        ...
        Public Function GetDatePivot() As String
            ...
            If GetVal(posField.MSGLIBRE).Contains(datePivot) Then
                ...
                If lst.Length > 1 Then
                    ...
                    d = strdate
                    Return PlushTools.ClsDate.formatDate(d)
                End If
    ...
    Private Function isexistCoda(ByVal coda_name As String) As Boolean
        Dim sql As String
        Dim cpt As Integer
        sql = PlushData.ClsBatchDomiciliation.IsexistCoda(coda_name)
    ...
    Private Function GetInfoDB(ByVal lstDataCoda As List(Of Coda), _
        ...
        strinsertdataHeader = "values('" & name_coda & "',now(),now()," & PlushData.clsSession.user_id & ","
    ...
    Private Function LoadStruct(ByVal lstCodaType As List(Of Coda), ByVal filePath As String, ByRef index_EndVirement As Integer) As List(Of Coda)
        ...
        struct = PlushTools.clsFile.openFile(filePath)
    ...
    Private Sub updateDOMStructure(ByVal coda_filename As String)
        ...
        sql = PlushData.ClsBatchDomiciliation.GetBankAccountMovementWithCommunicationStructure(coda_filename)
        ...
        For Each dr As DataRow In dt.Rows
            ...
            If isstruct Then
                ...
                strinsertdata = strinsertdata + " user_modified = " & PlushData.clsSession.user_id
        ...
        Next
        If str <> "" Then
            PlushData.clsConnection.ExecuteNonQuery(str)
        End If
    ...
    Private Sub updateamount(ByVal coda_filename As String)
        ...
        sql = PlushData.ClsBatchDomiciliation.GetBankAccountMovementAmount(coda_filename)
        PlushData.clsConnection.ExecuteNonQuery(sql)
    End Sub
    ...
    Private Sub updateDomUnpaid(ByVal coda_filename As String) ', ByVal lst_datePivot As String)
        ...
        Try
            ...
            sql = PlushData.ClsBatchDomiciliation.getFlagReconduct()
            PlushData.clsConnection.ExecuteNonQuery(sql)

            ' flag status payment the error dom of coda's today temporary status 
            sql = PlushData.ClsBatchDomiciliation.getFlagUnpaidDomInexist()
            unpaid_inexistant = PlushData.clsConnection.ExecuteNonQuery(sql)
            unpaid_inexistant = unpaid_inexistant / 2

            ' flag status payment insolvent of coda's today ->  temporary status
            sql = PlushData.ClsBatchDomiciliation.getFlagUnpaidInsolvent()
            unpaid_insolvent = PlushData.clsConnection.ExecuteNonQuery(sql)

            unpaid_insolvent = unpaid_insolvent / 2

            ' flag status payment compte solde of coda's today ->  temporary status
            sql = PlushData.ClsBatchDomiciliation.getFlagUnpaidCompteSolde()
            unpaid_CompteSolde = PlushData.clsConnection.ExecuteNonQuery(sql)

            unpaid_CompteSolde = unpaid_CompteSolde / 2

            ' flag status payment Paid of coda's today not error and not insolvent
            sql = PlushData.ClsBatchDomiciliation.getFlagPaidDom()
            paid = PlushData.clsConnection.ExecuteNonQuery(sql)

            ' flag status payment recovery all payment error or insolvent 
            sql = PlushData.ClsBatchDomiciliation.getInsertPaymentOfflineRequest()
            PlushData.clsConnection.ExecuteNonQuery(sql)
            ...
            sql = PlushData.ClsBatchDomiciliation.getChangeStatuspaidAfterLongTime()
            longtime_DomWaiting = PlushData.clsConnection.ExecuteNonQuery(sql)
        ...
        Catch ex As Exception
            ...
            clsMsgError.InsertLogMsg(PlushData.clsMsgError.processType.Import_Payment, ex)
