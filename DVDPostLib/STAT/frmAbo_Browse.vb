...
#End Region
    ...
    Public Overrides Sub LoadDataSet()
        ...
        _SQLTxt = _
            ...
            " c.`customers_id`, c.`customers_gender`, c.`customers_firstname`, c.`customers_lastname`,ad.`entry_street_address`,ad.`entry_postcode`,ad.`entry_city`, if(c.`customers_dob` = '0000-00-00', Null, c.`customers_dob`) as customers_dob, c.`email`, " & _
            " c.`customers_default_address_id`, c.`customers_telephone`, c.`encrypted_password`, " & _
            " c.`customers_newsletter`, c.`customers_newsletterpartner`, " & _
            " c.`customers_abo`, c.`customers_abo_suspended`, c.`customers_abo_type`, " & _
            ...
            " if(c.`customers_abo_discount_recurring_to_date` = '0000-00-00', Null, c.`customers_abo_discount_recurring_to_date`) as customers_abo_discount_recurring_to_date, " & _
            " c.`customers_abo_payment_method`, " & _
            " p.`products_id`, p.`products_model`, p.`imdb_id`, p.`products_title`, p.`products_price`, " & _
            " if(p.`products_date_added` = '0000-00-00', Null, p.`products_date_added`) as products_date_added , if(p.`products_last_modified` = '0000-00-00', Null, p.`products_last_modified`) as products_last_modified, if(p.`products_date_available` = '0000-00-00', Null, p.`products_date_available`) as products_date_available, p.`products_series_id`, p.`products_series_number`, " & _
            " p.`products_date_available`, " & _
            " p.`products_type` , aa.id, aa.name, " & _
            " ac.activation_code, ac.comment as activation_cmt , " & _
            " dc.discount_code, dc.discount_text_en as discount_cmt1, dc.comment as discount_cmt2 , ag.activation_group_name , ac.campaign,c.created_at " & _
        ...
        PlushData.clsConnection.FillDataSet(objDS.Tables(TableName), _SQLTxt)
        'LoadAboCreationDate()
        LoadWishListSize()
        LoadPreviousAction()
            ...
            For Each _row In objDS.Tables(TableName).Rows
                ...
                If Not IsDBNull(_row("customers_id")) Then
                    ...
                    _SQLTxt = "SELECT abo.`Date` as abo_date, abo.`action` as action_id , a.name as action_name, " & _
                         ...
                         " order by date desc limit 1 "
                    PlushData.clsConnection.FillDataSet(_DS.Tables("abo"), _SQLTxt)
        ...
        If chkPreviousDiscount.Checked = True Then
            ...
            For Each _row In objDS.Tables(TableName).Rows
                ...
                If Not IsDBNull(_row("customers_id")) Then
                    ...
                    PlushData.clsConnection.FillDataSet(_DS.Tables("Discount_use"), _SQLTxt)
        ...
        If TBMailId.Text <> "" Then
            ...
            For Each _row In objDS.Tables(TableName).Rows
                ...
                If Not IsDBNull(_row("customers_id")) Then
                    ...
                    PlushData.clsConnection.FillDataSet(_DS.Tables("MailSent"), _SQLTxt)
    ...
    Public Sub LoadActionID()
        ...
        PlushData.clsConnection.FillDataSet(_DS.Tables("abo_action"), _SQLTXt)
        Dim _Row As DataRow
        lblActionID.Text = ""
